...
AlphabetCoder::AlphabetCoder(const AlphabetType &type)
: min_code_(0), max_code_(0) {
  Set(type);
}

bool AlphabetCoder::Set(const AlphabetType &type) {
  string letters("");
  ...
  max_regular_letter_code_ = static_cast<Code>(type.GetRegularLetters().length() - 1);
  ...
  decode_map_.resize(max_code_ + 2, toupper(unknown_letter_));
	decode_map_[max_code_ + 1] = '#';
...
void AlphabetCoder::Encode (const char *sequence, unsigned int length, Code *encoded_sequence) const {
...
void AlphabetCoder::Decode(const Code *encoded_sequence, unsigned int length, char *sequence) const {
  for (uint32_t i = 0; i < length; ++ i) {
...
bool AlphabetCoder::IsUnknown(char letter) const {
  ...
  return (code_map_[static_cast<int>(l)] == unknown_code_) && (l != unknown_letter_);
}

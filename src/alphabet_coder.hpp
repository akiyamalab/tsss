...
#ifndef ALPHABET_CODER_HPP
#define ALPHABET_CODER_HPP
...
public:
  ...
  AlphabetCoder()
  {}
  AlphabetCoder(const AlphabetType &type);

  virtual ~AlphabetCoder()
  {}

  bool Set(const AlphabetType &type);

  Code Encode(char letter) const;

  void Encode(const char *sequence_data, unsigned int length, Code *encoded_sequence) const;

  char Decode(Code code) const;

  void Decode(const Code *encoded_sequence, unsigned int length, char *sequence_data) const;

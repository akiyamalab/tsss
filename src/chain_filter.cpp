...
void ChainFilter::Filter(const AlphabetCoder::Code *query_sequence,
	...
	for (vector<Hit>::iterator hits_it = hits.begin(); hits_it != hits.end();
		...
		uint32_t diagonal = query_sequence_length - 1
				+ hits_it->database_position
				- hits_it->query_position;
		...
		if (chains_[offset].diagonal != diagonal
			...
			chains_[offset].query_end = hits_it->query_position;
...
bool ChainFilter::Connected(const AlphabetCoder::Code *query_sequence,
		...
		Hit &hit, int cutoff) {
	if (hit.query_position <= chain_query_end + 1) {
		return true;
	}

	uint32_t q_offset = hit.query_position;
	uint32_t d_offset = hit.database_position;
	int score = 0;
	int best_score = 0;
	int c = -cutoff;
